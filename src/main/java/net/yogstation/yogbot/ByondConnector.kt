...
class ByondConnector(private val config: ByondConfig) {
	...
	fun request(rawQuery: String): Result<Any?, String?> {
		...
		try {
			Socket(config.serverAddress, config.serverPort).use { socket ->
				...
				if (inputStream.read(headerBuffer) < 4) return Result.error("Returned insufficient data")
				if (headerBuffer[0].toInt() != 0x00 || headerBuffer[1] != 0x83.toByte()) return Result.error("Malformed response packet, " + headerBuffer[0] + " " + headerBuffer[1])
				val size = ByteBuffer.wrap(headerBuffer, 2, 2).short
				val bodyBuffer = ByteArray(size.toInt())
				if (inputStream.read(bodyBuffer) < size) return Result.error("Returned insufficient data")
				val bb = ByteBuffer.wrap(bodyBuffer)
				val type = bb.get()
				if (type.toInt() == 0x2A) {
					return Result.success(bb.order(ByteOrder.LITTLE_ENDIAN).float)
				}
				return if (type.toInt() == 0x06) {
					Result.success(StandardCharsets.UTF_8.decode(bb).toString())
				} else Result.error("Type is $type")
			}
		} catch (e: IOException) {
