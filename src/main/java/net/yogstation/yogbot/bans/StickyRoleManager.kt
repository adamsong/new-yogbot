...
class StickyRoleManager(
	...
	fun doLogout(member: Member?): Mono<*> {
		...
		try {
			databaseManager.yogbotDbConnection.use { connection ->
				connection.prepareStatement("INSERT IGNORE INTO user_sticky_roles (discord_id, role_id) VALUES (?, ?)")
					.use { statement ->
	...
	fun doLogin(member: Member): Mono<*> {
		...
		try {
			databaseManager.yogbotDbConnection.use { connection ->
				connection.prepareStatement("SELECT role_id FROM user_sticky_roles WHERE discord_id = ?")
					.use { statement ->
						statement.setLong(1, member.id.asLong())
